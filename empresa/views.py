...
def nova_empresa(request):
    if request.method == "GET":
        techs = Tecnologias.objects.all()  # filter('TITLE')
        return render(request, 'nova_empresa.html', {'techs': techs})  # HttpResponse('Estou em nova_empresa')
...
def empresas(request):
    ...
    if nome_filtrar:
        ...
        # __icontains -> verificação quanto à existência dos caracteres digitados em nome_filtrar
...
def excluir_empresa(request, id):
    ...
    # return HttpResponse("O valor passado na url foi: {id}")
    return redirect('/home/empresas')
