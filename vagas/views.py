...
def nova_tarefa(request, id_vaga):
    # return HttpResponse(id_vaga)
    
    titulo = request.POST.get('titulo')
    prioridade = request.POST.get("prioridade")
    data = request.POST.get('data')
    
    # TODO: Realizar as validações
...
def realizar_tarefa(request, id):
    ...
    if not tarefas_list.exists():
        messages.add_message(request, constants.ERROR,
                             'Realize somente tarefas válidas.')
...
def envia_email(request, id_vaga):
    ...
    if email.send():
        ...
        mail.save()
        messages.add_message(request, constants.ERROR, 'Não foi possível completar o envio do e-mail')
        return redirect(f'/vagas/vaga/{id_vaga}')
